...
int lock;   // 0 indicate unlocked, 1 indicates lock
bool waiting;
int key;
...
// Shared counter for roww allocation
volatile int C = 0;
...
void* multi(void* arg)
{
    ...
    while (C<N)
    {
        ...
        do
        {
            curr_C = C;
            st = curr_C;
        } while (!__sync_bool_compare_and_swap(&C, curr_C, curr_C + rowInc));
...
int main()
{
    ...
    ofstream outFile("out_atomic.txt");
